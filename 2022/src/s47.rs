...
fn a_star(map: &mut MapState, start: Node, exit_coords: (i64, i64), debug: bool) -> i64 {
...
pub fn main() {
  let input = input();
  let mut map = MapState::new(input.split_str("\n").map(|v| v.into()).collect::<Vec<_>>());
  let start_tile = (1i64, 0i64);
  let exit_tile = (map.map_size.0, map.map_size.1 + 1);
  let mut t = 0;
  t += a_star(&mut map, (start_tile.0, start_tile.1, t), exit_tile, false);
  t += a_star(&mut map, (exit_tile.0, exit_tile.1, t), start_tile, false);
  t += a_star(&mut map, (start_tile.0, start_tile.1, t), exit_tile, false);
  println!("{}", t);
}