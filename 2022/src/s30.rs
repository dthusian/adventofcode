...
fn optimize_graph(value: &HashMap<u16, i64>, adj: &HashMap<u16, Vec<u16>>) -> OptimizedGraph {
  ...
  let mut value2 = relevant.iter().map(|v| value[v]).collect::<Vec<_>>();
...
pub fn main() {
  ...
  for t in 1..=30 {
    for bitmask in 0..1 << num_nodes {
      for pos in 0..num_nodes {
        let current = state[t][bitmask][pos];
        if current.is_none() { continue; }
        let current = current.unwrap();
        // possible move: wait one minute
        max_assign(&mut state[t + 1][bitmask][pos],
                   Some(current + bitmask_to_flow(bitmask as u64, &flows)));
        ...
        for possible_move in 0..num_nodes {
          ...
          max_assign(&mut state[t + dist_to_dest as usize][bitmask][possible_move],
                     Some(current + bitmask_to_flow(bitmask as u64, &flows) * dist_to_dest));
        }
        // possible move: open the current valve
        max_assign(&mut state[t + 1][bitmask | (1 << pos)][pos],
                   Some(current + bitmask_to_flow(bitmask as u64, &flows)));
